...
import { featuredHuntingAccessories } from "../../../Data/productsData";

const FeaturedCollectionBanner = ({ onQuickAdd }) => {
  return (
    <section className="w-full bg-[#F6F2EC] py-16 sm:py-20 lg:py-28 px-6 lg:px-12 xl:px-16 border-t border-[#E5D8C8]">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="text-center max-w-[700px] mx-auto mb-12 sm:mb-16">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-normal text-[#3B2618] tracking-tight mb-4">
            Designed to Complete Every Expedition
          </h2>
          <p className="text-[#6B6258] text-base sm:text-lg font-normal leading-relaxed">
            Handcrafted leather accoutrements designed to elevate your field setup with elegance and utility.
          </p>
        </div>

        {/* 3 Featured Products Editorial Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-14">
          {featuredHuntingAccessories.map((item, index) => (
            <motion.div
              key={item.id}
        ...
        <div className="flex justify-center">
          <Link
            ...
            className="group inline-flex items-center gap-3 bg-[#3B2618] hover:bg-[#8B5A2B] text-[#FAF8F5] px-8 py-4 rounded-full text-xs font-semibold uppercase tracking-[0.2em] transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer shadow-md"
          >
            <span>Shop Hunting Collection</span>
