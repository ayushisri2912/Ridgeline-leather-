...
import GunSlingHero from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingHero";
import GunSlingIntro from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingIntro";
import GunSlingToolbar from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingToolbar";
import GunSlingFilterSidebar from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingFilterSidebar";
import GunSlingProductCard from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingProductCard";
import GunSlingCraftsmanship from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingCraftsmanship";
import GunSlingCompleteCollection from "../../Components/HuntingGearComponents/gubSlingsComps/GunSlingCompleteCollection";
import { gunSlingProducts } from "../../Data/productsData";
...
const GunSlingsPage = () => {
  ...
  const handleQuickAdd = (product, color, length) => {
    setCartToast({
      title: product.name,
      price: product.price,
      image: product.image,
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedLengths.length > 0) {
      result = result.filter(
        (p) => p.lengths && p.lengths.some((len) => selectedLengths.includes(len))
      );
    }
  ...
  return (
    <div className="bg-[#FAF8F5] min-h-screen text-[#3B2618] selection:bg-[#8B5A2B] selection:text-[#FAF8F5]">
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="bg-[#FFFDFC] border border-[#E5D8C8] rounded-2xl p-12 text-center my-6 flex flex-col items-center">
                ...
                <h3 className="font-serif text-2xl text-[#3B2618] font-normal mb-2">
                  No Gun Slings Match Selected Filters
                </h3>
                <p className="text-[#6B6258] text-sm max-w-md mb-6">
                  Try adjusting your price range, length preference, or category filters to explore the full collection.
                </p>
                <button
      ...
      <GunSlingCraftsmanship />

      {/* SECTION 6: RELATED PRODUCTS */}
      <GunSlingCompleteCollection onQuickAdd={handleQuickAdd} />
        ...
        <div className="fixed bottom-6 right-6 z-50 bg-[#FFFDFC] border border-[#E5D8C8] shadow-2xl rounded-2xl p-4 flex items-center gap-4 animate-in slide-in-from-bottom duration-300 max-w-md">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-xs font-semibold text-[#3B2618]">
              ${cartToast.price} {cartToast.length && `• ${cartToast.length}`}
            </span>
          </div>
          <button
