...
import CartridgeBeltHero from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltHero";
import CartridgeBeltIntro from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltIntro";
import CartridgeBeltToolbar from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltToolbar";
import CartridgeBeltFilterSidebar from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltFilterSidebar";
import CartridgeBeltProductCard from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltProductCard";
import CartridgeBeltCraftsmanship from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltCraftsmanship";
import CartridgeBeltCompleteCollection from "../../Components/HuntingGearComponents/cartridgeBeltsComps/CartridgeBeltCompleteCollection";
import { cartridgeBeltProducts } from "../../Data/productsData";

const CartridgeBeltsPage = () => {
  ...
  const handleQuickAdd = (product, color, size) => {
    setCartToast({
      title: product.name,
      price: product.price,
      image: product.image,
  ...
  const filteredProducts = useMemo(() => {
    let result = [...cartridgeBeltProducts];
  ...
  return (
    <div className="bg-[#FAF8F5] min-h-screen text-[#3B2618] selection:bg-[#8B5A2B] selection:text-[#FAF8F5]">
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="bg-[#FFFDFC] border border-[#E5D8C8] rounded-2xl p-12 text-center my-6 flex flex-col items-center">
                ...
                <h3 className="font-serif text-2xl text-[#3B2618] font-normal mb-2">
                  No Cartridge Belts Match Selected Filters
                </h3>
                <p className="text-[#6B6258] text-sm max-w-md mb-6">
                  Try adjusting your price range, waist size, or category options to explore the full collection.
                </p>
                <button
      ...
      <CartridgeBeltCraftsmanship />

      {/* SECTION 6: COMPLETE THE COLLECTION */}
      <CartridgeBeltCompleteCollection onQuickAdd={handleQuickAdd} />
        ...
        <div className="fixed bottom-6 right-6 z-50 bg-[#FFFDFC] border border-[#E5D8C8] shadow-2xl rounded-2xl p-4 flex items-center gap-4 animate-in slide-in-from-bottom duration-300 max-w-md">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-xs font-semibold text-[#3B2618]">
              ${cartToast.price} {cartToast.size && `• Size ${cartToast.size}`}
            </span>
          </div>
          <button
...
export default CartridgeBeltsPage;
